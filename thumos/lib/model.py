...
class MCBD(nn.Module):
    ...
    def __init__(self, feature_dim, rgb):
        ...
        self.eca = eca_block(channel=self.hidden_dim_2d)
    ...
    def forward(self, x):
        ...
        if sigmoid_feature.size(1) != frame_feat['xc_feat'].size(1):
            sigmoid_features = sigmoid_features.view(x.shape[0], -1, 128)  # 假设您需要将其调整为 (batch_size, -1, 128)
    
            frame_feat['xc_feat'] = frame_feat['xc_feat'] + sigmoid_features
            frame_feat['xb_start'] = frame_feat['xb_start'] + sigmoid_features
            frame_feat['xb_end'] = frame_feat['xb_end'] + sigmoid_features
        ...
        confidence_map = self._boundary_matching_layer(x,N=n)
