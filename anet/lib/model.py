...
class BinomialDistribution:
    ...
    def calculate_probabilities(self):
        ...
        :return: 概率数组
        """
        x = np.arange(self.n + 1)  # 生成从0到n的所有整数
        probabilities = stats.binom.pmf(x, self.n, self.p)
        return probabilities

    def find_max_probability(self):
        """
        找到概率最大时的横坐标（即成功次数）
        :return: 概率最大时的横坐标和对应的概率
        """
        probabilities = self.calculate_probabilities()
        max_prob_index = probabilities.argmax()  # 找到概率最大的索引
        #max_prob = probabilities[max_prob_index]  # 获取最大概率
...
class MCBD(nn.Module):
    def __init__(self, feature_dim):
        ...
        self.expand = 4
        self.tfblock = TransformerBlock(d_points=self.hidden_dim_1d, d_model=self.hidden_dim_1d, k=4)
        
        self.interp1d_masks = {}
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.distribution = BinomialDistribution(p=0.8)
        
        # Base Module
        self.x_1d_b = nn.Sequential(
            nn.Conv1d(self.feature_dim, self.hidden_dim_1d, kernel_size=3, padding=1, groups=4),
        ...
        self.x_2d_p = nn.Sequential(
            ...
            nn.Conv2d(self.hidden_dim_2d, self.hidden_dim_2d, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(self.hidden_dim_2d, 4, kernel_size=1),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        x4 = self.x_1d_s(base_feature).squeeze(1)
        xb_start, xb_end, xc = torch.split(x4, 1, 1)
        
        pooled_feature = F.max_pool1d(x, kernel_size=2, stride=2)  # 最大池化
        # pooled_feature = F.avg_pool1d(base_feature, kernel_size=2, stride=2)  # 均值池化

        sigmoid_feature = torch.sigmoid(pooled_feature)
    
        max_value = torch.max(sigmoid_feature, dim=-1)[0]
        
        self.distribution.p = max_value.mean().item() 
        n = self.distribution.find_max_probability()
        
        if n not in self.interp1d_masks:
            mask = self._get_interp1d_mask(n)
            self.interp1d_masks[n] = mask.to(x.device)
            
        confidence_map = self.x_1d_p(x)
        confidence_map = self._boundary_matching_layer(confidence_map,N=n)
        confidence_map = self.x_3d_p(confidence_map).squeeze(2)
        net_feat = self.x_2d_p(confidence_map)
        prop_start = net_feat[:, 0:1].contiguous()
        prop_end = net_feat[:, 1:2].contiguous()
        iou = net_feat[:, 2:].contiguous()
        output_dict = {
            'xc': xc,
            'xb_start': xb_start,
            'xb_end': xb_end,
    ...
    def _boundary_matching_layer(self, x,N):
        mask = self.interp1d_masks[N]
        if mask.device != x.device:
            self.interp1d_masks[N] = mask.to(x.device)
            mask = self.interp1d_masks[N]
        input_size = x.size()
        out = torch.matmul(x, mask).reshape(input_size[0],input_size[1],self.num_sample,self.tscale,self.tscale)
        return out

    def _get_interp1d_bin_mask(self, seg_xmin, seg_xmax, tscale, num_sample, num_sample_perbin,N):
        ...
        for idx in range(num_sample):
            ...
            for sample in bin_samples:
                ...
                if int(sample_down) <= (tscale - 1) and int(sample_down) >= 0:
                    bin_vector[int(sample_down)] += 1 - sample_decimal    
                for j in range(N):
                    if int(sample_upper)+j<= (tscale - 1) and int(sample_upper) >= 0:
                        bin_vector[int(sample_upper)] += sample_decimal / (N)
            bin_vector = 1.0 / num_sample_perbin * bin_vector
            p_mask.append(bin_vector)
        p_mask = np.stack(p_mask, axis=1)
        return p_mask

    def _get_interp1d_mask(self,N):
